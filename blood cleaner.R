...
# As of September 23: 15/1765, top 0.85%
# As of October 2: 15/1798, top 0.83%
# As of October 4: 15/1810, top 0.83%
...
# New variables: need to explain what they're about
training.df$total_span <- training.df$months_first-training.df$months_last # Highest if indiv first donated a while ago and donated recently; lowest (0) if indiv's first donation was also last
training.df$avg_span <- training.df$total_span/training.df$donations # Highest if total_span high and donations low; middle if total_span high and donations high OR total_span low and donations low; lowest if total_span low and donations high; 0 if first donation was also last
...
training.df$onedone <- ifelse(training.df$total_span==0, training.df$onedone <- 1, training.df$onedone <- 0) # 1 if first donation was also last (one and done)
training.df$last_first <- training.df$months_last/training.df$months_first # Highest (1) if first donation was also last; lowest (0) if just donated in past month
training.df$lf_donations <- training.df$last_first/training.df$donations # last_first scaled by donations; higher if few donations, lower if more donations
training.df$span_first <- training.df$total_span/training.df$months_first # Complement of last_first; highest (1) if just donated in past month, lowest (0) if first donation was also last
training.df$sf_donations <- training.df$span_first/training.df$donations # span_first scaled by donations; if nonzero, higher if few donations, lower if more donations
training.df$periods <- training.df$months_first/2 # Number of opportunities indiv had within span of data to donate blood (assuming these are just whole blood donations, which can't be true. oh well.)
training.df$dperper <- training.df$donations/training.df$period # Average donations per periods within data span
training.df$freq <- ifelse(training.df$donations>=5, training.df$freq <- 1, training.df$freq <- 0) # "Frequent" donor; 5 or more total donations (most indivs in both datasets fall into 5 and under bracket)
...
submission2 <- data.frame(cbind(test.df$X, test.df$March2007))
colnames(submission2) <- c("", "Made Donation in March 2007")
head(submission2)
summary(submission2)
write.csv(submission2, file="complicated mess.csv", row.names=F) # Best so far	
